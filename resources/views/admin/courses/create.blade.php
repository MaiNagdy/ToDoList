    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course_days') ? 'is-invalid' : '' }}" type="number" name="course_days" id="course_days" value="{{ old('course_days', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Course::COURSE_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('course_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="course_type_{{ $key }}" name="course_type" value="{{ $key }}" {{ old('course_type', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('training_type') ? 'is-invalid' : '' }}" name="training_type" id="training_type">
                    ...
                    @foreach(App\Models\Course::TRAINING_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('training_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
