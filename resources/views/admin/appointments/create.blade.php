    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('session_number') ? 'is-invalid' : '' }}" type="number" name="session_number" id="session_number" value="{{ old('session_number', '1') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Appointment::SESSION_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('session_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="session_type_{{ $key }}" name="session_type" value="{{ $key }}" {{ old('session_type', 'عملي') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('session_date') ? 'is-invalid' : '' }}" type="text" name="session_date" id="session_date" value="{{ old('session_date') }}" required>
